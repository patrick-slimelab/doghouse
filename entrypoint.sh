...
# Configure tool deny list (matrix-search plugin and tools.aliases no longer exist in schema)
echo "[doghouse] Configuring tools"
...
gosu scoob env HOME=/home/scoob OPENCLAW_STATE_DIR=/home/scoob/.openclaw OPENCLAW_CONFIG_PATH=/home/scoob/.openclaw/openclaw.json /usr/local/bin/openclaw config set messages.groupChat.mentionPatterns [] --json || true
...
EOF
    ...
    sleep 1
    echo "[doghouse] Matrix Indexer process launch attempted"
